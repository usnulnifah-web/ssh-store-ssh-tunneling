...
import {assertLicense} from './license-agent-check.js';
...
async function createAccount({username,days}){await assertLicense();if(!validUsername(username))throw new Error('username tidak valid');if(!validDays(days))throw new Error('days harus 1 sampai 365');const db=await load();if(db[username])throw new Error('username sudah ada');const pass=password(),expires=expiry(days);await command('/usr/sbin/useradd',['-m','-s','/usr/sbin/nologin',username]);try{await command('/usr/sbin/chpasswd',[],`${username}:${pass}\n`);await command('/usr/sbin/usermod',['-e',expires,username])}catch(e){await command('/usr/sbin/userdel',['-r',username]).catch(()=>{});throw e}db[username]={username,expires,createdAt:new Date().toISOString(),status:'active'};await save(db);return {username,password:pass,expires,status:'active'}}
async function extendAccount({username,days}){await assertLicense();if(!validUsername(username)||!validDays(days))throw new Error('parameter tidak valid');const db=await load();if(!db[username])throw new Error('akun tidak ditemukan');const next=expiry(days);await command('/usr/sbin/usermod',['-e',next,username]);db[username].expires=next;db[username].status='active';await save(db);return db[username]}
async function setStatus(username,status){await assertLicense();if(!validUsername(username)||!['active','suspended'].includes(status))throw new Error('parameter tidak valid');const db=await load();if(!db[username])throw new Error('akun tidak ditemukan');if(status==='suspended')await command('/usr/sbin/usermod',['-L',username]);else await command('/usr/sbin/usermod',['-U',username]);db[username].status=status;await save(db);return db[username]}
async function deleteAccount(username){await assertLicense();if(!validUsername(username))throw new Error('username tidak valid');const db=await load();if(!db[username])throw new Error('akun tidak ditemukan');await command('/usr/sbin/userdel',['-r',username]);delete db[username];await save(db);return {deleted:true,username}}
